...
export async function PATCH(request: Request) {
  ...
  if (input.dayOfMonth !== undefined && (!Number.isInteger(input.dayOfMonth) || input.dayOfMonth < 1 || input.dayOfMonth > 31)) {
    return Response.json({ error: "日期需在 1-31 之间" }, { status: 400 });
  }
