...
export default function HomePage() {
  ...
  // A task can opt into the weekly plan even when its parent project does not.
  // Keep the dashboard aligned with the server-side /api/week selection rules.
  const weekProjectTasks = projectTasks.filter(
    (task) =>
      !task.done &&
      !task.scheduledDate &&
  ...
  const weekProjects = projects.filter(
    (project) =>
      ...
      weekProjectTasks.some((task) => task.projectId === project.id)
  );
  const weekOpenCount =
    weekTasks.filter((task) => !task.done).length +
    weekProjectTasks.length;
