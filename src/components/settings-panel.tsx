...
const blankSettings: PublicSettings = {
  ...defaultSettings,
  hermesTokenLast4: null,
  hermesTokenCreatedAt: null,
  hermesTokenRotatedAt: null,
  hermesTokenRevokedAt: null,
  hermesTokenAvailable: false,
  ...
  const resetHermesToken = async () => {
    ...
    try {
      const response = await fetch("/api/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ hermesTokenAction: "reset" }),
      });
      const result = (await response.json()) as PublicSettings & { hermesApiToken?: string };
      if (!response.ok || typeof result.hermesApiToken !== "string") {
        setNotice("Token 重置失败");
        return;
      }
      setOneTimeHermesToken(result.hermesApiToken);
      setSettings(result);
      setNotice("新 Token 只显示这一次，请立即复制并保存");
    } catch {
      setNotice("Token 重置失败，请检查服务器连接");
    } finally {
      setSaving(false);
    }
  };

  const revokeHermesToken = async () => {
    setSaving(true);
    try {
      const response = await fetch("/api/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ hermesTokenAction: "revoke" }),
      });
      if (response.ok) {
        setOneTimeHermesToken(null);
        setSettings((current) => ({
          ...current,
          hermesTokenAvailable: false,
          hermesTokenRevokedAt: Date.now(),
        }));
        setNotice("Token 已撤销");
      } else {
        setNotice("Token 撤销失败");
      }
    } finally {
      setSaving(false);
    }
          ...
          <aside className="fixed bottom-3 right-3 top-3 z-[61] w-[min(92vw,470px)] overflow-y-auto rounded-[26px] border border-white/10 bg-bg-secondary p-5 shadow-[-18px_0_60px_rgba(0,0,0,0.5)] sm:bottom-5 sm:right-5 sm:top-5 sm:p-6">
            ...
            <section className="mb-6 rounded-2xl border border-white/[0.08] bg-card/40 p-4">
              ...
              <p className="mb-3 text-[11px] leading-5 text-text-muted">
                ...
                Token 只在生成或重置成功后显示一次，服务器只保存不可逆哈希。
              </p>
              <div className="mb-3 rounded-xl bg-black/20 p-3 text-xs text-text-secondary">
                {settings.hermesTokenAvailable
                  ? `当前 Token 末四位：${settings.hermesTokenLast4 ?? "未知"}`
                  : "当前没有可用 Token"}
              </div>
              {oneTimeHermesToken && (
                <div className="mb-3 rounded-xl border border-accent-green/30 bg-accent-green/[0.06] p-3">
                  <p className="mb-2 text-[10px] text-accent-green">仅显示一次的新 Token</p>
                  <code className="block break-all font-mono text-xs text-text-primary">{oneTimeHermesToken}</code>
                  <button
                    onClick={() => {
                      void navigator.clipboard?.writeText(oneTimeHermesToken).then(() => setNotice("Token 已复制"));
                    }}
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() => void resetHermesToken()}
                  className="rounded-full border border-accent-green/25 bg-accent-green/[0.06] px-3 py-2 text-xs text-accent-green"
                >
                  {settings.hermesTokenAvailable ? "重置 Token" : "生成 Token"}
                </button>
                {settings.hermesTokenAvailable && (
                  <button
                    onClick={() => void revokeHermesToken()}
                    className="rounded-full border border-red-400/25 bg-red-400/[0.06] px-3 py-2 text-xs text-red-300"
                  >
                    撤销 Token
                  </button>
                )}
